...
export function TransactionModal({ isOpen, onClose, type }: TransactionModalProps) {
  ...
  const handleMpesaDeposit = async () => {
    ...
    if (isNaN(numAmount) || numAmount < 19) { toast({ title: "Invalid Amount", description: "Minimum is KES 19 (~$0.15)", variant: "destructive" }); return; }
  ...
  const SuccessScreen = ({ label }: { label: string }) => (
    <div className="flex flex-col items-center px-2 py-6">
      <div className="relative flex justify-center items-center mb-6" style={{ width: 90, height: 90 }}>
        ...
        <div className="relative z-10 flex justify-center items-center rounded-full" style={{ width: 56, height: 56, backgroundColor: '#0ECB81' }}>
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5" /></svg>
        </div>
      </div>
      <div className="text-base font-medium text-foreground mb-2">{label} Successful</div>
      <div className="flex items-baseline justify-center mb-8">
        <span className="text-[32px] font-bold text-foreground">{lastAmount || '0.00'}</span>
        <span className="text-lg font-medium text-foreground ml-0.5">USD</span>
      </div>
      <div className="w-full rounded-lg p-5 mb-8 bg-secondary">
        <div className="flex justify-between mb-4 text-[13px]"><span className="text-muted-foreground">Merchant</span><span className="font-medium text-foreground">Crypto Wave</span></div>
        <div className="flex justify-between mb-4 text-[13px]"><span className="text-muted-foreground">Date</span><span className="font-medium text-foreground">{new Date().toLocaleTimeString('en-US', { hour12: false })}</span></div>
        <div className="flex justify-between mb-4 text-[13px]"><span className="text-muted-foreground">Method</span><span className="font-medium text-foreground">{lastMethod}</span></div>
        <div className="flex justify-between text-[13px]"><span className="text-muted-foreground">Amount</span><span className="font-medium text-foreground">{lastAmount} USD</span></div>
      </div>
      <button onClick={onClose} className="w-full py-4 rounded font-medium text-[15px] mb-6 transition-colors" style={{ backgroundColor: '#FCD535', color: '#1E2329' }}>
        Back to Wallet
      </button>
    </div>
  );
